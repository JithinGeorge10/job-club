...
function page() {
  ...
  const onSubmit = async (data: login) => {
    const response = await axios.post(`${AUTH_SERVICE_URL}/admin-login`, data, {
      headers: {
    ...
    <>
      ...
      <div className="flex justify-center items-center min-h-screen bg-cover bg-center"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-lg bg-black p-8 rounded-lg shadow-lg">
          <h2 className="text-3xl font-semibold text-white text-center mb-6">Admin Login <span className="text-green-400">{`_JobClub.`}</span></h2>
