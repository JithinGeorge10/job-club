...
const MyJobsPage = () => {
    ...
    useEffect(() => {
        const fetchJobDetails = async () => {
            if (!userEmail) return;
            try {
                const response = await axios.get(`${COMPANY_SERVICE_URL}/interviewCompanies?id=${userEmail}`, {
                    headers: { 'Content-Type': 'application/json' },
                    withCredentials: true,
                });
                if (response.data?.interviewCompanies) {
                    setJobDetails(response.data.interviewCompanies);
                }
            } catch (error) {
        ...
        <div className="bg-black text-white font-sans min-h-screen">
            ...
            <main className="container mx-auto py-8 px-4 min-h-[70vh]">
                ...
                <div className="flex justify-center space-x-4 mb-6">
                    ...
                    <button className="text-lg font-semibold border-b-2 border-white">Interview</button>
                </div>
